...
export async function handleContactStateEvent(
  ...
  if (!conv) {
    ...
    // the per-sede AI bot. The previous trigger above only fires on
    // assignee events, so no event triggers our welcome path, and the
    // customer's first message gets ghosted (they have to send a SECOND
    // message for the AI to engage).
    //
    // Empirically (Railway 2026-06-26 PM), Respond.io does NOT emit
    // `contact.updated` for this customer-field-only change, so we widen
    // the net to ANY contact-state event we receive on a no-conv contact.
    // `maybeSendWorkflowAutoWelcome` is internally idempotent (in-flight
    // lock + existing-conv check + Branch-set guard inside getContact),
    // so this is safe — contacts whose Branch isn't set yet exit early
    // with a log line, contacts that already have a conv exit early too.
    log.info(
      { event, contactId },
      "auto-welcome candidate: no-conv contact-state event — running welcome path (Steve 2026-06-26 broad trigger)",
    );
    await maybeSendWorkflowAutoWelcome({
      contactId,
      newAssignee: null,
